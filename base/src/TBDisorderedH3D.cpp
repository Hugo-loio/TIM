...
TBDisorderedH3D::TBDisorderedH3D(TBModel model) : TBCleanH(model){
  nDisHop = 0;
  nDisOnSite = 0;
...
TBDisorderedH3D::~TBDisorderedH3D(){
  deleteDisArrays();
  delete[] isDisHop;
  delete[] isDisOnSite;
  delete[] indexDisHop;
...
void TBDisorderedH3D::setDisHop(int nDisHop, int * hop){
  deleteDisArrays();
  for(int i = 0; i < nHop; i++){
    isDisHop[i] = false;
    indexDisHop[i] = 0;
...
void TBDisorderedH3D::setDisOnSite(int nDisOnSite, int * onSite){
  deleteDisArrays();
  for(int i = 0; i < nOnSite; i++){
    isDisOnSite[i] = false;
    indexDisOnSite[i] = 0;
...
void TBDisorderedH3D::setSize(int * l){
  deleteDisArrays();
  TBCleanH::setSize(l);
}

void TBDisorderedH3D::deleteDisArrays(){
...
void TBDisorderedH3D::createDisArrays(){
...
void TBDisorderedH3D::fillHopWrapper(cx_mat & res, int hopIndex, double *k, int dim, int addI, int addJ, int addN, int * startI){
  fillHop<cx_mat>(res, hopIndex, k, dim, addI, addJ, addN, startI);
}

void TBDisorderedH3D::fillHopWrapper(sp_cx_mat & res, int hopIndex, double *k, int dim, int addI, int addJ, int addN, int * startI){
  fillHop<sp_cx_mat>(res, hopIndex, k, dim, addI, addJ, addN, startI);
}

void TBDisorderedH3D::fillOnSiteWrapper(cx_mat & res, int onSiteIndex, double * k, int dim, int addN, int * startI){
  fillOnSite<cx_mat>(res, onSiteIndex, k, dim, addN, startI);
}

void TBDisorderedH3D::fillOnSiteWrapper(sp_cx_mat & res, int onSiteIndex, double * k, int dim, int addN, int * startI){
  fillOnSite<sp_cx_mat>(res, onSiteIndex, k, dim, addN, startI);
}

template <class mat> void TBDisorderedH3D::fillOnSite(mat & res, int onSiteIndex, double * k, int dim, int addN, int * startI){
  int e = onSiteIndex;
  int j,n;
...
template <class mat> void TBDisorderedH3D::fillHop(mat & res, int hopIndex, double * k, int dim, int addI, int addJ, int addN, int * startI){
  complex<double> t;
  int j,n,q,r;
...
template <class mat> void TBDisorderedH3D::fillDis(mat & res, complex<double> *** w, complex<double> phase, int n, int * incN, int * start, int * end, int dim, int addI, int addJ, int * startI){
  ...
  while(i[dim] == val){
    ...
    res(n + addI, n + addJ) += w[i[rOrder[0]]][i[rOrder[1]]][i[rOrder[2]]]*phase;

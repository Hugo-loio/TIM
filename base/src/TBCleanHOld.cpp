...
TBCleanHOld::TBCleanHOld(TBModel model) : model(model), Hamiltonian(model.getNDim()){
  nOrb = model.getNOrb();
  nHop = model.getNHop();
...
void TBCleanHOld::delete_meshes(){
  if(nBulk != NULL){
...
void TBCleanHOld::setSparse(bool val){
  isSparse = val;
}

void TBCleanHOld::setSize(int * l){
  if(l != NULL){
    for(int i = 0; i < nDim; i++){
      this->l[i] = l[i];
    }
...
void TBCleanHOld::nextPoint(int depth, int * point, bool up){
  if(depth == nRDim-1){
    //increase coordinate
...
sp_cx_mat TBCleanHOld::spH(double * k){
  int size; 
